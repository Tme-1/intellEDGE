  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-10">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        ...
        <div className="mb-4">
          <button onClick={addGpaCourse} className="bg-blue-500 text-white px-4 py-2 rounded">Add</button>
        </div>
        <ul className="mb-4">
          {gpaCourses.map((c: any, i: number) => (
            <li key={i} className="flex gap-4 items-center border-b py-1">
              <span>{c.name}</span>
              <span>Grade: {c.grade}</span>
              <span>Unit: {c.creditUnit}</span>
            </li>
          ))}
        </ul>
        <div className="mb-4">
          <button onClick={calculateGpa} className="bg-green-500 text-white px-4 py-2 rounded mr-2">Calculate GPA</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="flex gap-4 mb-4">
          {semesters.map((_, idx) => (
            <button
              key={idx}
              className={`px-4 py-2 rounded-lg border font-bold mr-2 ${currentSemester === idx ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
          ...
          <button onClick={addSemester} className="px-4 py-2 rounded-lg bg-green-500 text-white font-bold">+ Add Semester</button>
        </div>
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Course Name</label>
        ...
        <div className="mb-4">
          <button onClick={addCgpaCourse} className="bg-blue-500 text-white px-4 py-2 rounded">Add</button>
      </div>
      <ul className="mb-4">
      ...
      <div className="mb-4">
        <button onClick={calculateCgpa} className="bg-green-500 text-white px-4 py-2 rounded mr-2">Calculate CGPA</button>
        </div>
